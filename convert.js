...
prompt.get(['password'], function(err, result) {
   ...
   db.connect(function(err) {
      if (err) {
         ...
         db.query("select * from entries", function(err, rows, cols) {
            rows.forEach(function(row) {
               var clearUser = AESDecryptCtr(row.user, pwd, 256);
               var clearPassword = AESDecryptCtr(row.password, pwd, 256);
               var cipherUser = AES.encrypt(clearUser, pwd).toString();
               var cipherPassword = AES.encrypt(clearPassword, pwd).toString();

               db.query("update entries set " +
                        "user = ?, " +
                        "password = ? " + 
                        "where id = ?", [cipherUser, cipherPassword, row.id], function() {
                  console.log(row.name, clearUser, clearPassword, cipherUser, cipherPassword);
               });
            });
         });
      }
